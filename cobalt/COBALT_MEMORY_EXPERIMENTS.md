...
| `CobaltMemPaTuning` | 08 | PartitionAlloc thread-cache multiplier halved (low-end path). NOTE: compiled out on evergreen-arm-hardfp-rdk — `PA_CONFIG(THREAD_CACHE_SUPPORTED)` is false there, so the flag is a no-op on that target (the build proved the guarded block doesn't compile); the 08b slot-span gn arg is the remaining PA lever on RDK | 3–8 MB where thread cache exists | component (blink wtf partitions) |
